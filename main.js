...
getData("/genre/movie/list").then((res) =>
  reload_genres(res?.data?.genres, box, btn)
);
...
getData("/discover/movie").then((res) => {
...
let enter_actor_page = document.querySelector(".enter_actor_page");
let enter_actor_page_two = document.querySelector(".enter_actor_page_two");

enter_actor_page.onclick = () => {
  location.assign("/pages/actor/index.html");
};
enter_actor_page_two.onclick = () => {
  location.assign("/pages/actor/index.html");
};
